...
router.put('/propuestaIntercambio/:id', (req, res) => {
    const { id } = req.params
    const { productoOfrecido, productoDeseado, usuarioOfrecido, usuarioDeseado, estado, fecha, calificoOfrecido, calificoDeseado ,registrado} = req.body
    IntercambioSchema.updateOne({ _id: id }, 
    {$set: { productoOfrecido, productoDeseado, usuarioOfrecido, usuarioDeseado, estado, fecha, calificoOfrecido, calificoDeseado ,registrado}})
    .then((data) => res.json(data)).catch((error) => res.json({message: error}))
})
